...
def parse_medicine_column(cell_content, global_cols):
    """实现精准物理位置映射：药名、数值、天数分别在原位对齐"""
    if pd.isna(cell_content): return {}, [], "", ""

    # 1. 基础清洗（类似CLEAN），保留原始长度
    ...
    for meta in parts_meta:
        ...
        if m:
            try:
                ...
                # C. 维测状态判定
                status = "合并" if dose_key in parsed_data else ("复用" if dose_key in global_cols else "新增")
                logs.append(f"[{status}] {dose_key}")
                parsed_data[dose_key] = parsed_data.get(dose_key, 0) + days

                # D. 【核心：物理锚点回填】
                # 1. 药名回填 (在原药名起始位置)
                d_start = meta['start'] + m.start('drug')
                for i, char in enumerate(drug_clean):
                    if d_start + i < meta['start'] + m.end('drug'):
                        aligned_chars[d_start + i] = char
                
                # 2. 数值回填 (在原数值起始位置)
                v_start = meta['start'] + m.start('val')
                for i, char in enumerate(v_str):
                    if v_start + i < meta['start'] + m.end('val'):
                        aligned_chars[v_start + i] = char

                # 3. 单位回填 (在原单位起始位置)
                u_start = meta['start'] + m.start('unit')
                for i, char in enumerate(unit_norm):
                    if u_start + i < len(aligned_chars):
                        aligned_chars[u_start + i] = char

                # 4. 天数回填 (在原天数起始位置)
                # 寻找数字部分在 part 中的相对起始位
                days_match = re.search(r'\d+', meta['text'][m.start('days'):])
                if days_match:
                    t_start = meta['start'] + m.start('days') + days_match.start()
                    day_display = f"*{days}天"
                    for i, char in enumerate(day_display):
                        if t_start - 1 + i < meta['end']: # -1 是为了把 * 号对齐到原频度或 * 的位置
...
def main():
    ...
    for index, row in df.iterrows():
        ...
        for k, v in medicine_info.items(): row_dict[k] = v
        processed_rows.append(row_dict)

    global_drug_cols.sort()
    print("\n" + "="*30 + " 2. 药品种类统计 " + "="*30)
    print(f"总计识别种类: {len(global_drug_cols)}")
    for i, col in enumerate(global_drug_cols, 1): print(f"{i:03d}. {col}")
